...
* _Для того чтобы запустить приложение из docker-контейнера, сначала вставьте команду:_
```
docker run gr3pr1/mse
```
...
1. Запуск генератора с заданным зерном
   >В результате выполнения данной команды будет сгенерирован файл `db_tree.txt`, в котором будет представлена сгенерированная база в данных в формате `.json` файла, со структурой, которая представлена в файле `example.json`. Если не задать сид, то БД будет сгенерированна со случайным сидом.
```commandline
